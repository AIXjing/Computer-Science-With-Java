public class RecursiveSquares {
  ...
  public static void draw(int n, double x, double y, double length) {
    if (n == 0) return;
    double x1 = x - length / 2, x2 = x + length / 2;
    double y1 = y - length / 2, y2 = y + length / 2;
    drawSquare(x, y, length / 2);
    draw(n - 1, x1, y2, length / 2.0);
    draw(n - 1, x2, y2, length / 2.0);
    draw(n - 1, x1, y1, length / 2.0);
    draw(n - 1, x2, y1, length / 2.0);
  }
