...
public class BarChart {
  ...
  private static Color[] initColors() {
    ...
    for (int i = 0; i < hex20.length; i++) colors[i] = Color.decode(hex20[i]);
    return colors;
  }

  /**
   * Sets the maximum x-value of this bar chart (instead of having it set automatially). This method
   * is useful if you know that the values stay within a given range.
   *
  ...
  /**
   * Adds a bar to the bar chart. The length of a bar is proportional to its value. The bars are
   * drawn from top to bottom in the order they are added. All bars from the same category are drawn
   * with the same color.
   *
  ...
  /**
   * Removes all of the bars from this bar chart (but keep the color scheme). This method is
   * convenient when drawing an animated bar chart.
   */
  public void reset() {
